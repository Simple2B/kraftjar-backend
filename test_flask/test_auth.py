...
def test_register(client):
    ...
    with mail.record_messages() as outbox:
        ...
        user = db.session.query(m.Admin).filter_by(email=TEST_EMAIL).first()
        assert user
        ...
        user_db = db.session.scalar(m.Admin.select().where(m.Admin.email == TEST_EMAIL))
        assert user_db
...
def test_forgot(client):
    ...
    with mail.record_messages() as outbox:
        ...
        user_db: m.Admin = db.session.scalar(m.Admin.select().where(m.Admin.email == TEST_EMAIL))
        assert user_db
